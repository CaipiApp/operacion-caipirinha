<script lang="ts">
	...
	onMount(() => {
		...
		$page.url.searchParams.delete('cuil');
...
>
	...
	<div class="flex flex-col items-center gap-4 mb-4 max-w-xs text-center">
		...
		<h1 class="mb-2">{$t('errorPageTitle')}</h1>

		{#if $page.status === 404}
			<p class="font-popLight">{$t('errorPageTitle404')}</p>

			<button class="btn text-center mt-4" on:click={handleClick404}>{$t("errorPageGoHome")}</button>
		{:else}
			<p class="font-popLight">
				{#if $page?.error?.message}
					{$page?.error?.message}
				{:else}
					{$t('errorPageDefaultText')} <br /> {$t('errorPageDefaultSubText')}
				{/if}
			</p>

			<div class="flex flex-col w-full items-center gap-4 mt-4">
				<button class="btn text-center" on:click={handleClick}> {$t('errorPageRetry')} </button>
				<a class="btn-light text-center" href={link} target="_blank"> {$t('errorPageContact')} </a>
			</div>
		{/if}
	</div>
